...
gdzie $t = k \cdot T_p$ ($T_p$ oznacza okres próbkowania):
\begin{itemize}
...
\end{equation}
W implementacji cyfrowej algorytm został zdyskretyzowany z~okresem próbkowania $T_p$. Dla uchybów regulacji zdefiniowanych jako $e_{\theta}(k) = \theta_{\mathrm{ref}} - x_1(k)$ oraz $e_x(k) = x_{\mathrm{ref}} - x_3(k)$, człon proporcjonalny realizowany jest jako proste wzmocnienie uchybu. Człon całkujący przybliżono numerycznie metodą prostokątów (sumowanie uchybów w~czasie), wyposażając go dodatkowo w~mechanizm \textit{anti-windup}, który ogranicza wartość całki do zakresu $\pm 5{,}0$. 

Istotną modyfikację wprowadzono w~członie różniczkującym. Zamiast klasycznego obliczania różnicy uchybów ($\frac{\Delta e}{\Delta t}$), co może prowadzić do wzmacniania szumów kwantyzacji, zastosowano technikę różniczkowania sygnału wyjściowego (ang. \textit{derivative on measurement}). Wykorzystując fakt, że przy stałych wartościach zadanych pochodna uchybu odpowiada ujemnej prędkości obiektu, w~równaniach sterowania użyto bezpośrednio zmiennych stanu prędkości. Ostateczne równania różnicowe regulatora przyjmują postać:
\begin{align}
    u_{\theta}(k) &= K_{p,\theta} e_{\theta}(k) + K_{i,\theta} \sum_{j=0}^{k} e_{\theta}(j) T_p + K_{d,\theta} \left( -x_2(k) \right), \label{eq:pd_theta} \\
    u_{x}(k) &= K_{p,x} e_x(k) + K_{i,x} \sum_{j=0}^{k} e_x(j) T_p + K_{d,x} \left( -x_4(k) \right). \label{eq:pd_x}
\end{align}
...
rzędu zerowego (ZOH) dla czasu próbkowania $T_p$.
