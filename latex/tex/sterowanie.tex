...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[auto, node distance=1.5cm, >=Stealth]
        ...
        \draw [arrow] (pd_theta.east) -| node[above, font=\scriptsize] {$u_p$} (sum_u.north);
        \draw [arrow] (pd_x.east) -| node[below, font=\scriptsize] {$u_c$} (sum_u.south);
...
Najpierw próbowano metodą prób i błędów nastroić regulator w taki sposób, aby 
metryki $MSE$ i $MAE$ były jak najmniejsze oraz, żeby jakość sygnału sterującego była odpowiednia.
Pierwsze próby doboru parametrów metodą prób i błędów dawały bardzo słabe wyniki, gdyż obiekt wahadła na 
wózku wymaga bardzo precyzyjnych nastaw, a dobór parametrów metodą prób i błędów jest czasochłonny i nie gwarantuje optymalnych rezultatów. 
...
\begin{figure}[H]
    ...
    \caption{Źle dostrojony regulator PID ($K_{p,\theta}=-10$, $K_{i,\theta}=-1$,$K_{d,\theta}=-3$, 
...
% co skutkuje szybszym osiągnięciem zadanego punktu. Jednak sygnał sterujący dla 
...
% Motywuje to rozszerzenie struktury o~dodatkową pętlę PID z~możliwością włączenia członu całkującego.

\subsubsection{Struktura hybrydowa PID-LQR}

Klasa \texttt{PDLQRController} implementuje sterowanie
...
\end{equation}
Dodatkowy człon PID na~pętli pozycji (zrealizowany analogicznie do wzoru
...
następująco:
\begin{equation}
    \min_{\Delta U} J = \sum_{k=1}^{N} (\hat{x}_k - x_{\mathrm{ref}})^T Q (\hat{x}_k - x_{\mathrm{ref}}) + R \sum_{k=1}^{N_u} (\Delta u_k)^2,
\end{equation}
przy ograniczeniach:
\begin{align}
...
Gdzie:
\begin{itemize}
    \item $N = 12$ -- horyzont predykcji,
    \item $N_u = 4$ -- horyzont sterowania. Dla kroków $k \ge N_u$ 
    stosowane jest tzw.~\textit{blokowanie sterowania}, co oznacza, że przyrosty 
    sterowania $\Delta u_k = 0$ i~sygnał sterujący pozostaje stały: $u_k = u_{N_u-1}$. 
    Technika ta redukuje liczbę zmiennych decyzyjnych z~$N$ do~$N_u$, 
    ...
    \item $Q = \text{diag}([158,\; 41,\; 43,\; 20])$ -- macierz kar stanu,
...
($Q_{\theta}=158$) w~połączeniu z~niską karą za~zmianę sterowania ($R=0,086$)
...
\begin{equation}
    J = \sum_{k=1}^{N} (x_k - x_{\mathrm{ref}})^T Q (x_k - x_{\mathrm{ref}}) \;+\; R_{\Delta} \sum_{k=1}^{N_u} (\Delta u_k)^2 \;+\; R_{\mathrm{abs}} \sum_{k=1}^{N_u} (u_k)^2.
\end{equation}
...
W~badaniach przyjęto zoptymalizowane wagi: $Q = \text{diag}([158 , 41 , 43 , 20])$,
przy czym $R_{\Delta}=0{,}01$ i~$R_{\mathrm{abs}}=1$ dla wariantu priorytetyzującego 
jakość regulacji.
...
Wprowadzenie członu $R_{\mathrm{abs}} \sum_{k=1}^{N_u} u_k^2$ do~funkcji celu ma 
...
zmiany), parametr $R_{\mathrm{abs}}$ bezpośrednio penalizuje amplitudę siły, 
co przekłada się na~redukcję zużycia energii. Z~punktu widzenia optymalizatora,
przy wysokich wartościach $R_{\mathrm{abs}}$, każdy niuton siły generuje istotny
koszt, co może prowadzić do~nadmiernie zachowawczego działania regulatora.

Zależność między wartością $R_{\mathrm{abs}}$ a~jakością regulacji ma charakter 
silnie nieliniowy. Dla~zbyt dużych wartości ($R_{\mathrm{abs}} > 5$) optymalizator
preferuje minimalizację energii kosztem jakości regulacji, co w~skrajnych przypadkach
prowadzi do~utraty stabilności. Dla~zbyt małych wartości ($R_{\mathrm{abs}} \approx 0$)
układ zachowuje się identycznie jak podstawowy MPC, tracąc potencjalne korzyści
...
\begin{figure}[H]
    ...
    \caption{Regulator MPC-J2 z~ręcznie zmniejszoną karą ($R_{\mathrm{abs}}=5$).}
...
\begin{figure}[H]
    ...
    \caption{Zoptymalizowany regulator MPC-J2 ($R_{\mathrm{abs}}=1$).}
    \label{fig:mpcj2_opt}
\end{figure}
...
Funkcja kosztu przyjmuje postać analogiczną do~NMPC:
\begin{equation}
    J = \sum_{k=1}^{N} (\hat{x}_k - x_{\mathrm{ref}})^T Q (\hat{x}_k - x_{\mathrm{ref}}) 
    + R \sum_{k=0}^{N_u-1} (\Delta u_k)^2,
\end{equation}
gdzie $\Delta u_k = u_k - u_{k-1}$ oznacza przyrost sterowania, a~dla $k \geq N_u$ 
stosuje się blokowanie sterowania ($\Delta u_k = 0$).
...
Proces strojenia regulatora LMPC przebiegł sprawnie, wykorzystując doświadczenia
z~uruchomienia regulatorów LQR oraz NMPC. Przyjęcie wag zbliżonych do~tych
stosowanych w~LQR pozwoliło na~uzyskanie poprawnych wyników już w~pierwszych próbach.
Ostatecznie zdecydowano się na~następującą konfigurację:
\begin{itemize}
    \item Horyzont predykcji: $N=12$ (identyczny jak w~NMPC),
    \item Horyzont sterowania: $N_u=4$,
    \item Macierz wag stanu: $Q = \text{diag}([15,0,\; 1,0,\; 15,0,\; 1,0])$,
    \item Waga sterowania: $R = 0,1$.
\end{itemize}
...
Na~Rys.~\ref{fig:lmpc_opt} przedstawiono odpowiedź układu na~skokową zmianę pozycji
zadanej wózka. Układ zachowuje się stabilnie, płynnie dochodząc do~wartości
zadanej. Widoczne jest działanie ograniczeń sygnału sterującego (nasycenie)
w~początkowej fazie ruchu, co jest kluczową przewagą nad klasycznym LQR.
