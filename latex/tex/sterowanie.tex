...
Proces doboru nastaw dla regulatora PD-PD przebiegał w~sekwencji prób, mających na~celu empiryczne wyznaczenie wzmocnień zapewniających stabilną pracę układu.

Początkowo przyjęto losowe, niewielkie wartości wzmocnień, co skutkowało brakiem stabilizacji wahadła. Mimo poprawnej implementacji algorytmu, obiekt po~krótkim czasie przewracał się, co wskazywało na~zbyt małą energię sygnału sterującego w~stosunku do~dynamiki nieliniowej układu.

\noindent \textit{TODO: Wstawić wykres (Screen 1) pokazujący szybkie przewrócenie się wahadła przy złych nastawach.}

W~kolejnym kroku zastosowano metodę dekompozycji. Najpierw dobierano wzmocnienia pętli stabilizacji ($K_{p,\theta}, K_{d,\theta}$) dla utrzymania pionu (ignorując pozycję), a~następnie ostrożnie dodawano wzmocnienia pętli pozycji ($K_{p,x}, K_{d,x}$). Uzyskano stabilizację, jednak układ charakteryzował się powolnym powrotem do~punktu pracy i~znacznymi oscylacjami, co jest typowe dla strojenia ręcznego bez precyzyjnej optymalizacji.

\noindent \textit{TODO: Wstawić wykres (Screen 2) ze strojenia ręcznego - stabilnie, ale słabo.}

Ostatecznie wykorzystano zaimplementowany w~katalogu \texttt{/tuning} algorytm ewolucji różnicowej (\texttt{scipy.optimize.differential\_evolution}). Funkcja kosztu minimalizowała całkę kwadratu uchybu (ISE) oraz karę za~sterowanie. Wynikowe nastawy zapewniły agresywną, ale precyzyjną stabilizację z~szybkim tłumieniem zakłóceń.

\noindent \textit{TODO: Wstawić wykres (Screen 3) z nastawami zoptymalizowanymi - idealne działanie.}
...
Dobór wag dla regulatora LQR również charakteryzował się ewolucyjnym podejściem do~problemu optymalizacji wskaźnika jakości.

W~pierwszej fazie przyjęcie jednostkowej macierzy diagonalnej $Q=I$ oraz $R=1$ okazało się niewystarczające. Mimo teoretycznej stabilności wynikającej z~rozwiązania równania CARE, wahadło wykonywało bardzo duże wychylenia, a~wózek wielokrotnie wyjeżdżał poza dopuszczalny zakres roboczy toru. Świadczyło to o~zbyt małej karze nałożonej na~uchyb kątowy.

\noindent \textit{TODO: Wykres (Screen 1) dla $Q=I, R=1$ - inercyjna reakcja.}

Następnie przeprowadzono strojenie ręczne metodą prób i~błędów (zgodnie z~regułą Brysona). Ręczne zwiększanie kar za~wychylenie kąta ($Q_{\theta}$) poprawiło sztywność wahadła. Udało się ustalić zestaw wag zapewniający stabilną pracę, choć czas regulacji był wciąż niezadowalający, a~reakcja na~zakłócenia powolna.

\noindent \textit{TODO: Wykres (Screen 2) dla wag Brysona.}

W~ostatnim etapie zastosowano optymalizację numeryczną. Algorytm genetyczny poszukiwał optymalnych elementów diagonalnych macierzy $Q$ oraz skalara $R$, minimalizując czas regulacji. Zoptymalizowane wagi (w szczególności wysoka kara $Q_{\theta} \approx 69.44$) znacznie przewyższyły jakością sterowania nastawy dobrane heurystycznie.

\noindent \textit{TODO: Wykres (Screen 3) dla LQR po optymalizacji.}
...
Dla regulatora MPC kluczowym wyzwaniem był dobór horyzontu predykcji oraz macierzy wag, determinujących zachowanie układu w~stanie nieustalonym.

Początkowe ustawienie zbyt krótkiego horyzontu predykcji ($N_{\mathrm{p}} < 5$) prowadziło do~niestabilności układu zamkniętego. Regulator ,,nie widział'', że rozpędzając wózek w~celu korekcji kąta, nie zdąży wyhamować przed upadkiem wahadła lub osiągnięciem końca toru.

\noindent \textit{TODO: Wykres niestabilnego MPC (krótki horyzont).}

Zwiększenie horyzontu do $N_{\mathrm{p}}=10$ w~ramach korekty ręcznej ustabilizowało proces. Dodatkowa manipulacja wagami $Q$ pozwoliła na~uzyskanie poprawnego sterowania, jednak koszt obliczeniowy był wysoki, a~przebiegi wciąż wykazywały niepożądane przeregulowania.

\noindent \textit{TODO: Wykres MPC strojonego ręcznie.}

Automatyzacja procesu strojenia przy użyciu skryptu \texttt{tune\_mpc.py} pozwoliła na~znalezienie kompromisu między długością horyzontu a~wagami. Algorytm optymalizacyjny wskazał $N_{\mathrm{p}}=12$ jako optimum dla tego modelu dyskretnego, zapewniając stabilność przy akceptowalnym czasie obliczeń.

\noindent \textit{TODO: Wykres zoptymalizowanego MPC - gładkie sterowanie.}
...
W~przypadku wariantu MPC-J2 analizowano nieliniowy wpływ parametru $R_{\mathrm{abs}}$ na~zachowanie układu.

Przyjęcie zbyt dużej wartości kary za~sterowanie bezwzględne ($R_{\mathrm{abs}}$) spowodowało, że regulator wykazywał tendencję do~pasywności. Wahadło przewracało się, ponieważ koszt energetyczny utrzymania go w~pionie przewyższał zysk wynikający z~małego błędu kąta w~funkcji celu.

\noindent \textit{TODO: Wykres upadku wahadła przy zbyt dużym R\_abs.}

Stopniowe, ręczne zmniejszanie parametru $R_{\mathrm{abs}}$ pozwoliło znaleźć punkt pracy, w~którym układ odzyskał stabilność przy zachowaniu relatywnej oszczędności energetycznej. Odpowiedź dynamiczna była jednak powolna i~zbyt asekuracyjna dla większych zakłóceń.

\noindent \textit{TODO: Wykres energooszczędny (wolny).}

Algorytm optymalizacyjny precyzyjnie dostroił $R_{\mathrm{abs}}$, minimalizując złożony wskaźnik kosztu (błąd + energia). Znaleziono ,,złoty środek'', w~którym układ stabilizuje się szybko, ale sterowanie pozbawione jest zbędnych oscylacji wysokoczęstotliwościowych, co przekłada się na~oszczędność energii.

\noindent \textit{TODO: Wykres optymalny MPC-J2.}
...
Strojenie rozmytego regulatora Fuzzy-LQR jest zadaniem złożonym ze względu na~dużą liczbę parametrów definiujących bazę reguł i~funkcje przynależności.

Błędne zdefiniowanie zbyt wąskich funkcji przynależności dla strefy ,,małego błędu'' skutkowało gwałtownym przełączaniem się regulatora na~agresywne reguły (chatter). Prowadziło to do~silnych drgań wokół punktu równowagi, co jest zjawiskiem niepożądanym w~rzeczywistych układach napędowych.

\noindent \textit{TODO: Wykres drgań (chatter) przy złych funkcjach przynależności.}

Opierając się na~literaturze \cite{art3}, dobrano ręcznie szerokości trójkątnych funkcji przynależności tak, aby przejście między strefami było płynne. Układ uzyskał stabilność asymptotyczną, jednak nie wykorzystywał w~pełni potencjału szybkiej reakcji na~duże zakłócenia, działając zachowawczo.

\noindent \textit{TODO: Wykres ręcznie dostrojonego Fuzzy-LQR.}

Ostatecznie, dedykowany skrypt \texttt{tune\_fuzzy\_lqr.py} posłużył do~optymalizacji wag pojedynczych reguł oraz parametrów kształtu funkcji przynależności. Uzyskano nieliniową powierzchnię sterowania, która łączy zalety miękkiego sterowania LQR w~pobliżu zera z~maksymalną siłą reakcji przy dużych wychyleniach.

\noindent \textit{TODO: Wykres optymalny Fuzzy-LQR (powierzchnia sterowania lub przebieg).}
