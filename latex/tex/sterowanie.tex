...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[auto, node distance=1.5cm, >=Stealth]
        ...
        \node [below=3pt of sum_theta.south, font=\tiny] {$-$};
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[auto, node distance=1.4cm, >=Stealth]
        ...
        \draw [arrow] ($(plant.east) + (0.3,0)$) -- ++(0,-2.4) -| (sum_lqr.south);
        \node [below=3pt of sum_lqr.south, font=\tiny] {$-$};
        
        % Sprzężenie zwrotne PD (x) - od głównej linii sprzężenia
        \coordinate (fb_junction) at ($(sum_lqr.south) + (0,-0.7)$);
        \draw [arrow] (fb_junction) -- ++(-1.2,0) |- (sum_pd.south);
...
regułą Brysona. Reguła ta postuluje, że elementy diagonalne macierzy $Q$ i~$R$ 
powinny być odwrotnie proporcjonalne do~kwadratów maksymalnych dopuszczalnych 
wartości odpowiednich zmiennych stanu i~sterowania, tj. $Q_{ii} = 1/x_{i,\max}^2$
oraz $R = 1/u_{\max}^2$. Ręczne zwiększanie kar za~wychylenie kąta ($Q_{\theta}$) poprawiło
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[auto, node distance=1.4cm, >=Stealth]
        ...
        \draw [arrow] ($(plant.east) + (0.3,0)$) -- ++(0,-1.8) -| node[pos=0.25, right, font=\scriptsize] {$x(t)$} ($(opt.south) + (0.6,0)$);
