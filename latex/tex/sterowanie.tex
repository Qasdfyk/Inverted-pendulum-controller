...
regulator wyznacza sygnał sterujący $\mathrm{u(k)}$ (siłę przyłożoną do wózka)
na~podstawie aktualnego wektora stanu
$x(k) = [\theta, \dot{\theta}, x, \dot{x}]^T$ oraz wartości zadanych
$x_{\mathrm{ref}}$.
...
Jest to podejście dekomponujące problem na~dwa podzadania:
...
\end{equation}
Definiując uchyby regulacji w $k$-tej chwili próbkowania jako $e_{\theta}(k) = \theta_{\mathrm{ref}} - \theta(k)$ 
oraz $e_x(k) = x_{\mathrm{ref}} - x(k)$, prawo sterowania w implementacji cyfrowej
przyjmuje postać dyskretną.
\begin{align}
    u_{\theta}(k) = K_{p,\theta} e_{\theta}(k) + K_{i,\theta} \sum_{j=0}^{k} e_{\theta}(j) \Delta t - K_{d,\theta} \frac{\theta(k) - \theta(k-1)}{\Delta t}, \label{eq:pd_theta} \\
    u_{x}(k) = K_{p,x} e_x(k) + K_{i,x} \sum_{j=0}^{k} e_x(j) \Delta t - K_{d,x} \frac{x(k) - x(k-1)}{\Delta t}. \label{eq:pd_x}
\end{align}
gdzie $\Delta t$ oznacza okres próbkowania.
...
Wstępne próby doboru metodą prób i~błędów (Rys.~\ref{fig:pid_manual}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=12cm]{images/tuning/pdpd_1_manual.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10cm]{images/tuning/pdpd_3_opt.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10cm]{images/tuning/pdpd_2_integral_bad.png}
    \caption{Regulator PID z optymalnymi nastawami ($K_{p,\theta}=-40$, $K_{i,\theta}=-1$, $K_{d,\theta}=-8$, 
...
Ostateczny regulator PID charakteryzuje się szybkim osiąganiem wartości zadanej, małymi błędami $MSE$ i $MAE$
...
Problem LQR polega na~znalezieniu prawa sterowania $u(k) = -K x(k)$, które 
minimalizuje wskaźnik jakości:
\begin{equation}
    J = \sum_{0}^{\infty} \left( x(k)^T Q x(k) + u(k)^T R u(k) \right),
\end{equation}
...
zestaw wag zapewniający stabilną pracę.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=12cm]{images/tuning/pdlqr_1_bad.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10cm]{images/tuning/pdlqr_2_manual.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10cm]{images/tuning/pdlqr_3_opt.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10cm]{images/tuning/mpc_1_bad.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10cm]{images/tuning/mpc_2_manual.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10.5cm]{images/tuning/mpcJ2_1_bad.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10.5cm]{images/tuning/mpcJ2_2_manual.png}
...
Najpierw przetestowano zestaw wag $Q=\text{diag}([1, 1, 1, 1])$, $R=0,1$. Jednak na~Rys.~\ref{fig:lmpc_opt} widać, że pozycja wózka nie stabilizuje się wystarczająco szybko.
Zdecydowano się w kolejnym kroku zwiększyć wagi w macierzy $Q$.
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10cm]{images/tuning/lmpc_bad.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth, height=10cm]{images/tuning/lmpc_opt.png}
    \caption{Przebiegi czasowe dla liniowego regulatora MPC ($N=12$, $N_u=4$, 
...
drgania przełączeniowe, jednak wzmocnienia reguł były zbyt słabe ($F_{\theta}=20$).
...
\begin{figure}[H]
    ...
    \caption{Zoptymalizowany regulator Fuzzy-LQR ($F_{\theta}=40$, 
...
(Rys.~\ref{fig:fuzzy_opt}). Algorytm wyznaczył wartość wzmocnienia
