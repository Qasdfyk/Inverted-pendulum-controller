...
\begin{figure}[H]
    ...
    \caption{Zoptymalizowany regulator PID ($K_{p,\theta}=-95$, $K_{i,\theta}=0$, $K_{d,\theta}=-14$, 
...
już pod uwagę wyniki uzyskane podczas strojenia z wykorzystaniem algorytmu ewolucyjnego.
...
Użyto solvera \texttt{L-BFGS-B} z~biblioteki \texttt{scipy.optimize} do rozwiązania problemu sterowania. 
Wybór tego solvera (zamiast \texttt{SLSQP} stosowanego 
w~NMPC) podyktowany był charakterem zadania -- przy braku dodatkowych ograniczeń 
nierównościowych (poza ograniczeniami typu \textit{box} na~sygnał sterujący) 
algorytm \texttt{L-BFGS-B} wykazuje wyższą efektywność obliczeniową.
