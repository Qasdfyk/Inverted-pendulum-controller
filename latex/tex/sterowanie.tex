...
Dobór nastaw dla regulatora PID został zrealizowany wieloetapowo, ewoluując
...
Warto jednak zauważyć, że w~strukturze hybrydowej PID-LQR, gdzie LQR zapewnia 
...
\begin{figure}[H]
    ...
    \caption{Zoptymalizowany regulator PID ($K_{p,\theta}=-95$, $K_{d,\theta}=-14$, 
    $K_{p,x}=-16$, $K_{d,x}=-14$).}
    \label{fig:pdpd_opt}
\end{figure}
...
\subsubsection{Analiza wpływu członu całkującego w~układzie PID-LQR}

Istotną różnicą między regulatorem PID (bez LQR) a~układem hybrydowym PID-LQR 
jest możliwość bezpiecznego zastosowania akcji całkującej ($K_i \neq 0$). 
Jak wykazano w~Rozdziale 4.1, dodanie członu całkującego do~samodzielnego 
regulatora PID prowadziło do~oscylacji i~potencjalnej niestabilności 
(Rys.~\ref{fig:pdpd_integral}), co jest zgodne z~obserwacjami w~literaturze 
\cite{Prasad2014}.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/tuning/pdlqr_4_with_ki.png}
    \caption{Regulator PID-LQR z~członem całkującym ($K_{p,x}=-2{,}5$, 
    $K_{i,x}=-1{,}0$, $K_{d,x}=-5$, $Q=\mathrm{diag}([1, 1, 500, 250])$, $R=1$).}
    \label{fig:lqr_with_ki}
\end{figure}

W~przypadku układu hybrydowego PID-LQR sytuacja jest diametralnie inna. 
Regulator LQR zapewnia optymalne tłumienie poprzez sprzężenie od~pełnego 
wektora stanu, co skutecznie kompensuje dodatkowe przesunięcie fazowe 
wprowadzane przez człon całkujący. Na Rysunku~\ref{fig:lqr_with_ki} 
przedstawiono odpowiedź układu z~włączonym członem całkującym 
($K_i = -1{,}0$). W~przeciwieństwie do~regulatora PID, gdzie identyczna 
konfiguracja powodowała oscylacje (por. Rys.~\ref{fig:pdpd_integral}), 
układ PID-LQR zachowuje pełną stabilność i~płynność przebiegów.

Korzyści z~zastosowania członu całkującego w~układzie PID-LQR obejmują:
\begin{itemize}
    \item \textbf{Eliminację uchybu ustalonego} -- akcja całkująca gwarantuje 
    zerowy błąd w~stanie ustalonym dla stałych wartości zadanych.
    \item \textbf{Lepszą kompensację zakłóceń} -- całkowanie błędu pozwala 
    na~skuteczniejsze eliminowanie wpływu stałych zakłóceń.
    \item \textbf{Zachowanie stabilności} -- LQR zapewnia wystarczający 
    zapas stabilności, aby układ tolerował dodatkową dynamikę integratora.
\end{itemize}

Obserwacja ta jest zgodna z~wynikami przedstawionymi w~pracy \cite{Prasad2014}, 
gdzie autorzy wykazali, że połączenie regulatora PID z~optymalnym sprzężeniem 
zwrotnym LQR pozwala uzyskać lepsze właściwości dynamiczne niż każda z~metod 
stosowana oddzielnie. W~niniejszej pracy potwierdzono eksperymentalnie, 
że struktura hybrydowa umożliwia bezpieczne wykorzystanie pełnego potencjału 
regulatora PID (włącznie z~członem całkującym), co nie jest możliwe 
w~konfiguracji bez LQR.
...
w~standardowej dystrybucji Pythona oraz zdolnością do~obsługi ograniczeń 
nierównościowych (wymaganych dla saturacji sterowania). Zadanie zdefiniowane jest
następująco:
\begin{equation}
...
$\Delta t=0{,}01$~s daje zaledwie $0{,}05$~s predykcji) jest niewystarczający, aby uchwycić pełną
...
predykcję na~$0{,}1$~s, co pozwoliło na~antycypację skutków podejmowanych działań 
...
\begin{figure}[H]
    ...
    $Q=\mathrm{diag}([158.4, 36.8, 43.4, 19.7])$, $R=0.086$).}
    \label{fig:mpc_opt}
\end{figure}
...
Algorytm optymalizacyjny wskazał $N_{\mathrm{p}}=12$ jako optimum dla tego modelu
dyskretnego, zapewniając stabilność przy akceptowalnym czasie obliczeń. Zoptymalizowane
...
zapewnia agresywną, ale gładką stabilizację.
...
narastał, próba korekcji wymagała agresywnego ruchu wózka, co z~kolei destabilizowało
...
\end{equation}
gdzie $w_i$ to stopień aktywacji $i$-tej reguły, a~$G = 0.37$ to globalne
...
na~agresywne reguły (chatter). Zakres $[-0.02, 0.02]$~rad oznacza, że już przy
wychyleniu wahadła o~około $1^{\circ}$ regulator przeskakiwał z~trybu ,,łagodnego''
(LQR) na~tryb ,,agresywny'' (duże wzmocnienia reguł rozmytych), a~przy powrocie
do~pionu natychmiast wracał. Prowadziło to do~silnych drgań wokół punktu równowagi,
...
chatter, jednak wzmocnienia reguł były zbyt słabe ($F_{\theta}=20$ vs. optymalne $167.9$).
...
\begin{figure}[H]
    ...
    \caption{Zoptymalizowany regulator Fuzzy-LQR ($F_{\theta}=167.9$, 
    $F_{\dot{\theta}}=5.3$, $F_x=19.8$, $F_{\dot{x}}=19.3$, $G=0.37$).}
    \label{fig:fuzzy_opt}
\end{figure}
...
(Rys.~\ref{fig:fuzzy_opt}). Algorytm wyznaczył znacząco wyższą wartość wzmocnienia
dla kąta ($F_{\theta}=167.9$), przy jednoczesnym zmniejszeniu globalnego wzmocnienia
($G=0.37$). Uzyskano nieliniową powierzchnię sterowania, która łączy zalety miękkiego 
