...
podczas gdy ,,czysty'' regulator PD zapewnia sztywne i~szybkie sterowanie.
...
(vs. $-1$). Algorytm ,,odkrył'', że agresywna reakcja na~błąd pozycji wózka pośrednio
...
\begin{figure}[H]
    ...
    \caption{Zoptymalizowany regulator PID-LQR ($Q=\mathrm{diag}([1, 1, 500, 250])$, 
    $R=1$, PID: $K_{p,x}=-1.5$, $K_{d,x}=-5$).}
    \label{fig:lqr_opt}
\end{figure}
...
agresywnie pilnuje pozycji wózka, co pośrednio wymusza stabilne trzymanie wahadła. 
Interesujące jest, że optymalizator ,,odkrył'' strategię przeciwną do~intuicyjnej: 
...
Rys.~\ref{fig:mpc_bad}) prowadziło do~niestabilności układu zamkniętego. Regulator 
,,nie widział'', że rozpędzając wózek w~celu korekcji kąta, nie zdąży wyhamować 
przed upadkiem wahadła lub osiągnięciem końca toru. Horyzont pięciu kroków (przy
...
(Rys.~\ref{fig:mpc_manual}) ustabilizowało proces. Regulator zyskał możliwość
,,patrzenia w~przyszłość'' na~$0{,}1$~s, co pozwoliło na~antycypację skutków swoich
działań. Dodatkowa manipulacja wagami $Q$ pozwoliła na~uzyskanie poprawnego 
...
Funkcja kosztu karała każdy niuton siły tak mocno, że optymalizator ,,wolał'' pozwolić
wahadłu upaść, niż ponieść wysoki koszt energetyczny ratowania go. W~efekcie układ
...
zdolności regulatora do~szybkiej reakcji. Znaleziono ,,złoty środek'', w~którym układ
...
(Rys.~\ref{fig:fuzzy_opt}). Algorytm znalazł zaskakująco wysoką wartość wzmocnienia
