...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{Schemat blokowy regulatora PD-PD o~strukturze równoległej.}
...
\begin{figure}[H]
    ...
    \caption{Zoptymalizowany regulator PD-PD ($K_{p,\theta}=-95$, $K_{d,\theta}=-14$, 
    $K_{p,x}=-16$, $K_{d,x}=-14$).}
    \label{fig:pdpd_opt}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Zoptymalizowany regulator PD-LQR ($Q=\mathrm{diag}([1, 1, 500, 250])$, 
    $R=1$, PD: $K_{p,x}=-1.5$, $K_{d,x}=-5$).}
    \label{fig:lqr_opt}
\end{figure}
...
$dt=0.02$s daje zaledwie 0.1s predykcji) jest niewystarczający, aby uchwycić pełną
...
,,patrzenia w~przyszłość'' na~0.2s, co pozwoliło na~antycypację skutków swoich
