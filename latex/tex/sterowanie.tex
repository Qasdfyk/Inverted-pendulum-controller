...
\begin{figure}[H]
    ...
    \caption{Regulator PD nastawiony ręcznie ($K_{p,\theta}=-10$, $K_{i,\theta}=-1$,$K_{d,\theta}=-3$, 
...
\begin{figure}[H]
    ...
    \caption{Regulator PD nastawiony ręcznie ($K_{p,\theta}=-40$, $K_{i,\theta}=-1$, $K_{d,\theta}=-8$, 
...
W~literaturze \cite{Prasad2014} sugeruje się, że struktura hybrydowa PID-LQR 
...
Z~tego powodu w~finalnej konfiguracji regulatora PID-LQR zdecydowano się 
na~wyłączenie członu całkującego ($K_i = 0$), pozostawiając strukturę PID-LQR 
(Rys.~\ref{fig:lqr_opt}).
...
Zaimplementowano również wariant Liniowego MPC (LMPC) w klasie \texttt{LinearMPCController} (plik \texttt{lmpc.py}). W odróżnieniu od nieliniowego MPC, ten regulator wykorzystuje uproszczony, zlinearyzowany model obiektu wokół punktu równowagi górnej ($\theta = 0$).

Dyskretny model liniowy uzyskiwany jest metodą ZOH (Zero-Order Hold). Problem sterowania sprowadza się do minimalizacji kwadratowej funkcji kosztu przy uwzględnieniu ograniczeń na sygnał sterujący. Dzięki linearyzacji, problem optymalizacji jest prostszy obliczeniowo niż w przypadku NMPC, co pozwala na szybsze wyznaczanie sterowania, choć odbywa się to kosztem dokładności modelu w większej odległości od punktu pracy. W implementacji wykorzystano solver \texttt{L-BFGS-B} do obsługi ograniczeń. LMPC stanowi kompromis pomiędzy klasycznym LQR (model liniowy, brak saturacji) a nieliniowym MPC (model dokładny, pełna obsługa ograniczeń).

\subsection{Regulator rozmyty wspomagany LQR (Fuzzy-LQR)}

Ostatnim zbadanym układem jest sterownik hybrydowy o~strukturze równoległej \texttt{TSFuzzyController}
(plik \texttt{fuzzy\_lqr.py}), łączący klasyczny, liniowy regulator LQR z~nieliniowym systemem
