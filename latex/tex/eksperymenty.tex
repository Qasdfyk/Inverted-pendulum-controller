...
\begin{enumerate}
    ...
    ($\varphi=0, x=0$) oraz ocena czasu regulacji i przeregulowań dla małych
    wychyleń od punktu równowagi.
    ...
    porywisty wiatr) generowana zgodnie z~procedurą opisaną w~Rozdziale 3. Test
    ten pozwala ocenić krzepkość (ang. \textit{robustness}) układu zamkniętego.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Czas regulacji $t_s$ (Settling Time)} -- Czas, po którym sygnał wyjściowy trwale wchodzi w kanał tolerancji (przyjęto $\pm \varepsilon$ od wartości zadanej) i już go nie opuszcza.
    ...
    \item \textbf{Wskaźnik tłumienia zakłóceń (Robustness/SNR)} -- W scenariuszach z wiatrem: stosunek wartości skutecznej (RMS) błędu regulacji do wartości skutecznej siły zakłócającej.
\end{itemize}
