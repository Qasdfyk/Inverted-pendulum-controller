...
testów symulacyjnych. Każdy z~zaimplementowanych regulatorów (PID-PID, PID-LQR, MPC, MPC-alt, Fuzzy-LQR, LMPC) poddany został badaniom w~następujących scenariuszach:
...
\begin{enumerate}
    ...
    W~eksperymencie zwiększono masę wahadła $m$ o~100\%, co odpowiada sytuacji,
    ...
    \begin{itemize}
        ...
        \item Rzeczywista masa wahadła: $m_{\mathrm{real}} = 0{,}253$~kg ($+100\%$).
    \end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{ISE (Integral of Squared Error)} -- Sumaryczne kryterium kwadratowe, będące miarą energii uchybu.
    \begin{equation}
        \mathrm{ISE} = \sum_{k=1}^{N} (y(k) - y_{\mathrm{ref}}(k))^2
    \end{equation}

    \item \textbf{IAE (Integral of Absolute Error)} -- Sumaryczne kryterium modułu błędu, akumulujące całkowity uchyb.
    \begin{equation}
        \mathrm{IAE} = \sum_{k=1}^{N} |y(k) - y_{\mathrm{ref}}(k)|
    \end{equation}

    \item \textbf{Energia sterowania L2 ($E_{u, L2}$)} -- Koszt kwadratowy sterowania, powiązany z energią elektryczną/mechaniczną.
    \begin{equation}
        E_{u, L2} = \sum_{k=1}^{N} u(k)^2
    \end{equation}

    \item \textbf{Energia sterowania L1 ($E_{u, L1}$)} -- Koszt absolutny sterowania (zużycie paliwa/zasobów).
    \begin{equation}
        E_{u, L1} = \sum_{k=1}^{N} |u(k)|
    \end{equation}
