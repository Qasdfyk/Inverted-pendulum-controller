...
\textbf{Rungego-Kutty czwartego rzędu (RK4)}. Wybór tej metody podyktowany 
...
\(\Delta t = 0{,}1\,\text{s}\). Wartość tę dobrano na~podstawie analizy 
dynamiki układu: dominująca stała czasowa niestabilnego modu wahadła 
wynosi $\tau \approx 1/\lambda_4 \approx 0{,}18$ s (gdzie $\lambda_4 \approx 5{,}49$ 
to dodatnia wartość własna macierzy stanu). Krok $\Delta t = 0{,}1$ s 
jest zatem wystarczająco mały, aby poprawnie odwzorować dynamikę układu 
(spełniony warunek $\Delta t < \tau$), a~jednocześnie pozwala na~efektywne
działanie numerycznych algorytmów optymalizacji MPC w~czasie zbliżonym 
do~rzeczywistego.
...
\end{equation}
gdzie $P$ oznacza moc zakłócenia, a~$T_{\mathrm{s}} = \Delta t$
to krok symulacji. W~przeprowadzonych eksperymentach przyjęto $P = 5{,}0$ W, 
co odpowiada wartości skutecznej siły wiatru rzędu $\pm 2{,}2$ N. 
Wartość ta została dobrana tak, aby zakłócenia stanowiły istotne wyzwanie 
dla układu sterowania, lecz nie przekraczały możliwości kompensacyjnych 
badanych regulatorów.
