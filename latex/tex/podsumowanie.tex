...
założone cele badawcze: zaimplementowano sześć różnych strategii sterowania
(PID-PID, PID-LQR, MPC, MPC-J2, Fuzzy-LQR, LMPC), przeprowadzono ich optymalizację
...
\begin{enumerate}
    ...
    zaledwie $0{,}05$ rad. W~warunkach nominalnych jest to rozwiązanie bardziej kosztowne
    energetycznie ($E_u \approx 2{,}86$) od klasycznych metod, jednak w~obecności zakłóceń
    okazuje się najbardziej efektywne ($E_u \approx 10{,}1$), tłumiąc błędy w~zarodku.
    ...
    \item \textbf{Uniwersalność PD-LQR.} Regulator hybrydowy PD-LQR, po~odpowiednim
    doborze wag ($Q_x = 500$), okazał się rozwiązaniem bardzo uniwersalnym.
    W~warunkach zakłóceń zewnętrznych osiągnął on wyniki porównywalne z~najlepszymi regulatorami,
    zużywając mniej energii ($E_u \approx 10{,}5$) niż sterowanie predykcyjne MPC ($E_u \approx 12{,}4$).
...
\begin{itemize}
    ...
    \item \textbf{Uproszczony model zakłóceń.} Przyjęty model zakłóceń zewnętrznych (filtrowany
