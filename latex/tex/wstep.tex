...
proste, układ ten wymaga zaawansowanych metod stabilizacji oraz 
...
od~klasycznych regulatorów PID (regulatory proporcjonalno-różniczkująco-całkujące) i~LQR (liniowy regulator kwadratowy), po~sterowanie predykcyjne (MPC), LMPC, 
...
łączy złożoność teoretyczną z~praktycznymi wyzwaniami inżynierskimi. 
Stanowi tym samym uniwersalne narzędzie do~nauki, testowania i~rozwijania 
metod stabilizacji systemów nieliniowych.

\subsection{Cel i~zakres pracy}

Celem pracy jest zaprojektowanie i~realizacja autorskiego 
...
(PID-PID, PID-LQR, MPC, MPC-J2, Fuzzy-LQR, LMPC). Kluczowa jest również możliwość weryfikacji 
odporności regulatorów poprzez generowanie addytywnych zakłóceń 
zewnętrznych. System powinien oferować narzędzia 
...
\begin{itemize}
  \item klasyczne (PID-PID),
  \item optymalne i~predykcyjne (PID-LQR, MPC, LMPC),
  \item sterowanie rozmyte (Fuzzy).
\end{itemize}

Wszystkie te algorytmy zostały uruchomione 
w~tym samym środowisku symulacyjnym. Dzięki temu możliwe było ich 
porównanie w~ujednoliconych warunkach testowych. Sprawdzono nie tylko, jak precyzyjnie 
utrzymują wahadło w~pionie, ale także jak radzą sobie z~zakłóceniami 
zewnętrznymi oraz ile energii zużywają podczas pracy.

W~ramach projektu zaimplementowano i~poddano badaniom następujące struktury:
\begin{enumerate}
    \item \textbf{Regulator PID-PID} --- reprezentujący klasyczne 
    podejście inżynierskie. Układ składa się z~dwóch równoległych pętli 
    sprzężenia zwrotnego typu PID (stabilizacja kąta i~pozycji), bez członu 
    całkującego ($K_i=0$).
    
    \item \textbf{Hybrydowy układ PID-LQR} --- struktura łącząca regulator 
    ...
    Służy on w~pracy jako nowoczesny punkt odniesienia, pozwalający ocenić, 
    jakie korzyści daje uwzględnienie ograniczeń sterowania i~dynamiki 
    układu bezpośrednio w~procesie optymalizacji.

    \item \textbf{Regulator LMPC} --- liniowy wariant sterowania predykcyjnego
    korzystający z modelu zlinearyzowanego, stanowiący kompromis obliczeniowy.
...
układu na~zakłócenia (szumy, siły zewnętrzne). 
...
W~pracy~\cite{Prasad2014} przedstawiono kompletny model nieliniowy obiektu, 
...
oraz Tatjewskiego~\cite{Tatjewski2016}. Omawiają one szczegółowo zagadnienia 
...
od~klasycznych paradygmatów PID i~LQR~\cite{Prasad2014}, przez ujęcia 
predykcyjne~\cite{Varghese2017, Tatjewski2016}, aż po~zaawansowane metody 
hybrydowe i~inteligentne~\cite{Nguyen2024}.
