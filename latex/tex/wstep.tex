...
od~klasycznych regulatorów PD/PID (regulatory proporcjonalno-różniczkujące/całkujące) i~LQR (liniowy regulator kwadratowy), po~sterowanie predykcyjne (MPC), 
...
(PD, PD-LQR, MPC, Fuzzy-LQR). Kluczowa jest również możliwość weryfikacji 
...
\begin{enumerate}
    \item \textbf{Regulator PD} --- reprezentujący klasyczne 
    podejście inżynierskie. Układ składa się z~dwóch równoległych pętli 
    sprzężenia zwrotnego typu PD (stabilizacja kąta i~pozycji), bez członu 
    całkującego ($K_i=0$), z~ograniczeniami sygnału sterującego.
    
    \item \textbf{Hybrydowy układ PD-LQR} --- struktura łącząca regulator 
    PD (w~pętli regulacji pozycji, bez członu całkującego) z~optymalnym regulatorem stanu LQR 
...
implementacyjne regulatorów: klasycznego PD, optymalnego LQR, 
