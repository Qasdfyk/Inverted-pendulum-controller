...
klasyczne, do których zaliczono równoległy układ PID oraz hybrydowy PID-LQR, oraz regulatory
...
zdecydowanie lepszy od regulatora PID ($T_s \approx 0.3$ s). PID-LQR działa agresywnie,
...
agresywny LQR zużywa więcej energii ($E_u \approx 1.48$) niż regulator PID o~łagodniejszej 
charakterystyce ($E_u \approx 0.85$), który charakteryzuje się dłuższym i~łagodniejszym 
dochodzeniem do~równowagi. Można zatem zaobserwować zmianę charakterystyki: PID stał się 
rozwiązaniem bardziej ekonomicznym w
stanie przejściowym, podczas gdy PID-LQR charakteryzuje się szybszą odpowiedzią dynamiczną.
...
$E_u \approx 0.56$ jest ponad dwukrotnie niższe od obecnego LQR ($1.48$) oraz niższe od PID,
...
Drugi scenariusz badawczy stanowił znacznie bardziej wymagający test. Do układu wprowadzono sygnał
...
przeciwdziałający wiatrowi. Oznacza to, że skuteczna kompensacja wychylenia kątowego nieuchronnie
...
kąt ($Max |\theta| \approx 0.060$ vs $0.068$ rad dla PID), ale przede wszystkim istotnie
redukuje dryf pozycji ($Max |x| \approx 0.22$ m vs $0.26$ m). Warto zauważyć, że
osiąga to przy niższym zużyciu energii ($E_u \approx 11.7$) niż regulator PID ($15.4$).
Wynika to z faktu, że optymalny regulator tłumi zakłócenie w~zarodku, nie pozwalając na
rozhustanie się układu, podczas gdy PID kompensuje skutki oscylacji, które sam dopuścił.
...
jest znaczący ($E_u \approx 85.9$). Sterownik ten charakteryzuje się bardzo agresywną strategią 
sterowania --- wykorzystuje maksymalną dostępną moc w~celu minimalizacji błędu regulacji.

Zupełnie inną charakterystykę w~obliczu wiatru wykazuje MPC. Ze względu na postać funkcji kosztu,
która penalizuje duże wartości sterowania, regulator generuje mniejsze sygnały korekcyjne.
Prowadzi to do pewnych, kontrolowanych wychyleń wahadła pod wpływem wiatru. Skutkuje to największym 
dryfem wózka w zestawieniu ($Max |x| \approx 0.40$ m), gdyż układ jest spychany przez wiatr, 
...
\begin{table}[h!]
    ...
    \begin{tabular}{|l|c|c|c|c|c|}
        \hline
        Wskaźnik & PID & PID-LQR & MPC & MPC-J2 & Fuzzy-LQR \\ \hline
...
Regulatory klasyczne (PID, PID-LQR) oraz rozmyty (Fuzzy-LQR) charakteryzują się
...
kroku symulacji ($\Delta t = 10$~ms), co potwierdza możliwość pracy MPC
