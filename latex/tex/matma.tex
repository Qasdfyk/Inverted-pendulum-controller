...
siła zakłócająca \(F_{\mathrm{w}}(t)\) przyłożona do~masy wahadła~\cite{art1}.
...
otrzymujemy:
\begin{equation}
    M \ddot{x} + m \left( \ddot{x} + l \cos\theta \cdot \ddot{\theta} - l \sin\theta \cdot \dot{\theta}^2 \right) = u + F_{\mathrm{w}}.
\end{equation}
...
\begin{equation}
    m l \cos\theta \left( \ddot{x} + l \cos\theta \ddot{\theta} - l \sin\theta \dot{\theta}^2 \right) - m l \sin\theta \left( -l \sin\theta \ddot{\theta} - l \cos\theta \dot{\theta}^2 \right) = m g l \sin\theta + F_{\mathrm{w}} l \cos\theta.
\end{equation}
...
\begin{equation}
    \ddot{x} = \frac{1}{m \cos\theta} \left( m g \sin\theta + F_{\mathrm{w}} \cos\theta - m l \ddot{\theta} \right),
\end{equation}
...
oraz zakłócenia \(F_{\mathrm{w}}\), przyjmują postać~\cite{art1}:
