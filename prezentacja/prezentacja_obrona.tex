...
\begin{frame}{Regulator Fuzzy-LQR (rozmyty Takagi–Sugeno)}
\begin{columns}
\begin{column}{0.55\textwidth}
\small
Interpolacja lokalnych regulatorów LQR:

\textbf{Reguły rozmyte:}
\[
R_i: \text{Jeśli } \theta \text{ jest } A_i \text{, to } u_i = -K_i x
\]

\textbf{Wyjście systemu:}
\[
u = \frac{\sum_{i=1}^{M} w_i \cdot (-K_i x)}{\sum_{i=1}^{M} w_i}
\]

gdzie $w_i = \mu_{A_i}(\theta)$ -- stopień przynależności

\textcolor{Success}{\textbf{+}} Adaptacja do stanu układu\\
\textcolor{Success}{\textbf{+}} Wysoka precyzja stabilizacji
\end{column}
